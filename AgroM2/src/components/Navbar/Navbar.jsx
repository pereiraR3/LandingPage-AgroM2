...
import { MenuIcon, XIcon } from '@heroicons/react/outline'; // Ícones para abrir/fechar o menu
...
const Navbar = () => {
  ...
  return (
    <div>
      {/* Navbar */}
      <nav className={`bg-yellow-600 py-4 px-6 flex justify-between items-center fixed top-0 left-0 w-full z-40 transition-transform duration-300 ease-in-out ${isOpen ? 'transform -translate-x-full' : ''}`}>
      ...
      {/* Sidebar */}
      <div className={`fixed top-0 left-0 h-full bg-yellow-600 w-64 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out md:hidden z-50`}>
